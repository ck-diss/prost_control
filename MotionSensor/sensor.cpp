...
int Sensor::ms_open() {
	...
	initialized = 1;
    //debug delay_ms(10000);
	return 0;
}

std::vector<float> Sensor::ms_update(const int imuAddr) {
    std::vector<float> vec(3);
    i2chandlerSensor.changeIMU(imuAddr);
//    qDebug() << "in ms_update";
    ...
    //edit: if was a while loop -> 15ms

//    qDebug() << "vor while";

    while (dmp_read_fifo(g,a,_q,&sensors,&fifoCount)!=0){vec[0] = vec[1] =vec[2] = 555;return vec;}; //gyro and accel can be null because of being disabled in the efeatures
...
uint8_t Sensor::GetGravity(VectorFloat *v, Quaternion *q) {
...
uint8_t Sensor::GetYawPitchRoll(float *data, Quaternion *q, VectorFloat *gravity) {
	// yaw: (about Z axis)
