...
#ifdef AK89xx_SECONDARY
	setup_compass();
    if (mpu_set_compass_sample_rate(40))
		return 1;
    printf("after compass setup!\n");
 ...
 *  @return     0 if successful.
 */
uint8_t mpu_get_temperature(float *data)
{
	uint8_t tmp[2];
	int16_t raw;
    ...
    //debug data[0] = (int32_t)((35 + ((raw - (float)st.hw->temp_offset) / st.hw->temp_sens)) * 65536L);
    data[0] = (float)(raw / 333.87 +21);
...
#ifdef AK89xx_SECONDARY
        mpu_set_compass_sample_rate(40);
...
/**
 *  @brief      Read from the DMP memory.mpu_get_accel_reg
...
//		return 1;
    ...
    if (i2c_read(st.hw->addr, st.reg->raw_compass, 8, tmp)){
        printf("error magnet reading");
        return 1;}
    printf("%i %i %i",tmp[0],tmp[1],tmp[2]);
    printf("in magnet sensor2a");
	...
	if ((tmp[7] & AKM_OVERFLOW) || (tmp[7] & AKM_DATA_ERROR))
		return 3;
#elif defined AK8963_SECONDARY
    //printf("in magnet sensor3");

    /* AK8963 doesn't have the data read error bit. */
    if (!(tmp[0] & AKM_DATA_READY) || (tmp[0] & AKM_DATA_OVERRUN)) {
        //printf("in magnet sensor4");
        return 2;}
    if (tmp[7] & AKM_OVERFLOW){
        //printf("in magnet sensor5");
        return 3;}
#endif
    printf("## Magnet read successfully!");
